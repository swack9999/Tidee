...
public class ChangeDepItemInvController implements Initializable {
	// Attributes
...
//			System.out.println("select * from `item` where `itemName` like '" 
//					+ SearchValue.getText() + "%' and `depNum` = '" + GlobalConstants.depNo + "'");
			ResultSet rs = conn.execStatement(false, "select * from `item` where `itemName` like '" 
					+ SearchValue.getText() + "%' and `depNum` = '" + GlobalConstants.depNo + "'");
	...
	public void LoadDatabase() throws SQLException {
		...
		Connector conn = new Connector();
		data= FXCollections.observableArrayList();
		ResultSet rs = conn.execStatement(false,"select * from `item` where `depNum` = '"+GlobalConstants.depNo
				+"'");
		while (rs.next()) {
			data.add(new TableItems(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getDouble(4),rs.getString(5),rs.getInt(6),rs.getInt(7),rs.getInt(8),
					rs.getString(9),rs.getString(10)));
